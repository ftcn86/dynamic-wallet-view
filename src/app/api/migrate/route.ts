 ...
 * This will create all the necessary tables for the official Pi Network implementation
 * 
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🏗️ [MIGRATION] Creating tables...');
    ...
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "user_sessions" (
      "id" TEXT NOT NULL,
    ...
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "payment_orders" (
      "id" TEXT NOT NULL,
    ...
    await prisma.$executeRaw`CREATE UNIQUE INDEX IF NOT EXISTS "user_sessions_sessionToken_key" ON "user_sessions"("sessionToken")`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS "user_sessions_sessionToken_idx" ON "user_sessions"("sessionToken")`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS "user_sessions_userId_isActive_idx" ON "user_sessions"("userId", "isActive")`;
    await prisma.$executeRaw`CREATE UNIQUE INDEX IF NOT EXISTS "payment_orders_paymentId_key" ON "payment_orders"("paymentId")`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS "payment_orders_paymentId_idx" ON "payment_orders"("paymentId")`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS "payment_orders_userId_paid_idx" ON "payment_orders"("userId", "paid")`;
    ...
    console.log('✅ [MIGRATION] Core tables created successfully');

    // Create legacy tables for existing functionality
    console.log('🏗️ [MIGRATION] Creating legacy tables...');

    // User Settings
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "user_settings" (
      "id" TEXT NOT NULL,
      "userId" TEXT NOT NULL,
    ...
    // Balance Breakdown
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "balance_breakdowns" (
      "id" TEXT NOT NULL,
      "userId" TEXT NOT NULL,
    ...
    // Unverified Pi Details
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "unverified_pi_details" (
      "id" TEXT NOT NULL,
      "userId" TEXT NOT NULL,
    ...
    // Badges
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "badges" (
      "id" TEXT NOT NULL,
      "name" TEXT NOT NULL,
    ...
    // User Badges
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "user_badges" (
      "id" TEXT NOT NULL,
      "userId" TEXT NOT NULL,
    ...
    // Team Members
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "team_members" (
      "id" TEXT NOT NULL,
      "userId" TEXT NOT NULL,
      "name" TEXT NOT NULL,
    ...
    // Node Data
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "node_data" (
      "id" TEXT NOT NULL,
      "userId" TEXT NOT NULL,
    ...
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "node_performance_history" (
      "id" TEXT NOT NULL,
    ...
    // Transactions
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "transactions" (
      "id" TEXT NOT NULL,
      "userId" TEXT NOT NULL,
      "date" TIMESTAMP(3) NOT NULL,
      "type" TEXT NOT NULL,
    ...
    // Balance History
    await prisma.$executeRaw`CREATE TABLE IF NOT EXISTS "balance_history" (
      "id" TEXT NOT NULL,
      "userId" TEXT NOT NULL,
      "date" TIMESTAMP(3) NOT NULL,
    ...
    // Create unique constraints and indexes for legacy tables
    await prisma.$executeRaw`CREATE UNIQUE INDEX IF NOT EXISTS "user_settings_userId_key" ON "user_settings"("userId")`;
    await prisma.$executeRaw`CREATE UNIQUE INDEX IF NOT EXISTS "balance_breakdowns_userId_key" ON "balance_breakdowns"("userId")`;
    await prisma.$executeRaw`CREATE UNIQUE INDEX IF NOT EXISTS "unverified_pi_details_userId_key" ON "unverified_pi_details"("userId")`;
    await prisma.$executeRaw`CREATE UNIQUE INDEX IF NOT EXISTS "user_badges_userId_badgeId_key" ON "user_badges"("userId", "badgeId")`;
    await prisma.$executeRaw`CREATE UNIQUE INDEX IF NOT EXISTS "node_data_userId_key" ON "node_data"("userId")`;
    await prisma.$executeRaw`CREATE UNIQUE INDEX IF NOT EXISTS "balance_history_userId_date_key" ON "balance_history"("userId", "date")`;

    // Create additional indexes
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS "transactions_userId_date_idx" ON "transactions"("userId", "date")`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS "transactions_userId_type_idx" ON "transactions"("userId", "type")`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS "notifications_userId_read_idx" ON "notifications"("userId", "read")`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS "notifications_userId_date_idx" ON "notifications"("userId", "date")`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS "balance_history_userId_date_idx" ON "balance_history"("userId", "date")`;

    // Add foreign key constraints for legacy tables
    ...
    console.log('📋 [MIGRATION] All tables in database:', allTables);
