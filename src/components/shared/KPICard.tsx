...
export function KPICard({ title, value, icon, footerValue, change, badgeText, badgeVariant = 'default' }: KPICardProps) {
  return (
    <Card className="bg-card/80 backdrop-blur-xl border border-border/60 shadow-sm hover:shadow-xl transition-all duration-300 min-h-[120px] flex flex-col">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 flex-shrink-0">
        <CardTitle className="text-xs sm:text-sm font-medium text-muted-foreground leading-tight">
          {title}
        </CardTitle>
        <div className="flex h-6 w-6 sm:h-8 sm:w-8 items-center justify-center rounded-md bg-primary/90 text-primary-foreground flex-shrink-0 shadow">
